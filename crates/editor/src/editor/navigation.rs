...
impl EditorEngine {
	pub fn move_left(&mut self, layout: &DocumentLayout) {
	...
	pub fn move_right(&mut self, layout: &DocumentLayout) {
		self.clear_pointer_anchor();
		match self.mode() {
	...
	pub fn move_vertical(&mut self, layout: &DocumentLayout, direction: isize) {
		self.clear_pointer_anchor();
		match self.mode() {
			EditorMode::Normal => {
	...
	pub fn move_line_edge(&mut self, layout: &DocumentLayout, to_start: bool) {
		self.clear_pointer_anchor();
		match self.mode() {
			EditorMode::Normal => {
				let Some(current) = self.active_selection(layout) else {
	...
	pub fn exit_insert(&mut self) -> ApplyResult {
