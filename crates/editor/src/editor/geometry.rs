...
pub fn span_rectangle(start: &LayoutCluster, end: &LayoutCluster) -> LayoutRect {
...
pub fn insert_cursor_rectangle(buffer: &Buffer, font_size: f32, text: &str, byte: usize) -> Option<LayoutRect> {
...
pub fn insert_cursor_block(buffer: &Buffer, font_size: f32, text: &str, byte: usize) -> Option<LayoutRect> {
	let geometry = insert_cursor_geometry(buffer, font_size, text, byte)?;
...
pub fn insert_selection_range(buffer: &Buffer, text: &str, byte: usize) -> Option<Range<usize>> {
...
pub fn selection_rectangles(layout: &DocumentLayout, range: &Range<usize>) -> Arc<[LayoutRect]> {
...
pub fn normal_selection_geometry(
...
fn insert_cursor_geometry(buffer: &Buffer, font_size: f32, text: &str, byte: usize) -> Option<InsertCursorGeometry> {
	...
	let y = (visual_lines_offset(cursor.line, buffer) + visual_line as f32).mul_add(line_height, -scroll.vertical);
