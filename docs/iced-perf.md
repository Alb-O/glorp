...
Status: still open at the architecture level. Viewport culling reduced the amount of work paid on each scroll redraw, but the cache still invalidates on scroll because the canvas path cannot cheaply re-translate cached geometry.
...
Status: still open and likely the next real bottleneck after the current low-risk fixes. Even with viewport culling, the `canvas::Text` layer still redraws the whole document text on scroll.
...
Status: partially addressed. Outline extraction is now gated by render mode, and the static canvas pass culls vector work to the visible viewport. The remaining offscreen cost is the `canvas::Text` path, which still draws whole-document text.
...
The next meaningful improvement is probably not another micro-optimization inside `canvas`. The larger win is to move document text rendering onto an `iced` paragraph/editor-style path with persistent buffers, or otherwise replace the current `canvas::Text` document layer.
